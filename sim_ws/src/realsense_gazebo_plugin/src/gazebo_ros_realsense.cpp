...
void GazeboRosRealsense::OnNewFrame(const rendering::CameraPtr cam,
  ...
  const std::map<std::string, sensor_msgs::Image>
    image_map = {
        {COLOR_CAMERA_NAME, this->image_msg_color_},
        {IRED1_CAMERA_NAME, this->image_msg_ir1_},
        {IRED2_CAMERA_NAME, this->image_msg_ir2_},
    };

  // copy data into image
  this->image_map[camera_id].header.frame_id = this->cameraParamsMap_[camera_id].optical_frame;
  this->image_map[camera_id].header.stamp.sec = current_time.sec;
  this->image_map[camera_id].header.stamp.nsec = current_time.nsec;
  ...
  fillImage(this->image_map[camera_id], pixel_format, cam->ImageHeight(),
            cam->ImageWidth(), cam->ImageDepth() * cam->ImageWidth(),
            reinterpret_cast<const void *>(cam->ImageData()));
  
//  //teste rgb pcl ERRO
//  if (camera_id=="color"){
//    fillImage(this->image_map, pixel_format, cam->ImageHeight(),
//              cam->ImageWidth(), cam->ImageDepth() * cam->ImageWidth(),
//              reinterpret_cast<const void *>(cam->ImageData()));
//
//  };
  ...
  auto camera_info_msg =
      cameraInfo(this->image_map[camera_id], cameras.at(camera_id)->HFOV().Radian());
  image_pub->publish(this->image_map[camera_id], camera_info_msg);
}
                                              ...
                                              uint32_t step_arg, void *data_arg)
{
  ...
  for (uint32_t j = 0; j < rows_arg; j++)
  {
    ...
    for (uint32_t i = 0; i < cols_arg; i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
    {
      ...
      uint8_t *image_src = (uint8_t *)(&(this->image_map[COLOR_CAMERA_NAME].data[0]));
      if (this->image_map[COLOR_CAMERA_NAME].data.size() == rows_arg * cols_arg * 3)
      {
        // color
        if (this->image_map[COLOR_CAMERA_NAME].encoding == sensor_msgs::image_encodings::RGB8) {
          iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 0];
          iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
          iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 2];
        }
        else if (this->image_map[COLOR_CAMERA_NAME].encoding == sensor_msgs::image_encodings::BGR8) {
          iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 0];
          iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
          iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 2];
        }
        else {
          throw std::runtime_error("unsupported colour encoding: " + this->image_map[COLOR_CAMERA_NAME].encoding);
        }
      }
      else if (this->image_map[COLOR_CAMERA_NAME].data.size() == rows_arg * cols_arg)
      {
