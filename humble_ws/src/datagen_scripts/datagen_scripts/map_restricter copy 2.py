...
class MapRestricterServer(Node):
    ...
    def apply_restriction(self):
        ...
        #Camera point
        point_1 = Point()
        point_1.x = float(math.floor((self.camera_pose.position.x - map_meta["origin"][0]) / map_meta["resolution"]))
        point_1.y = float(math.floor((self.camera_pose.position.y - map_meta["origin"][1]) / map_meta["resolution"]))
